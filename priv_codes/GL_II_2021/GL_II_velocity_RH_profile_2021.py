...
sys.path.insert(0, "/home/piotr/Piotr/parcel/plots/comparison")
sys.path.insert(0, "./")
...
w_list = [0.25, 1, 3, 5]
sstp_cond = [10]
mu = [0.15e-6, 0.01e-6, 0.54e-6, 0.14e-6]
N_tot = [90e6, 70e6, 10e7, 80e6]
stdev = [2.98, 1.38, 2.5, 1.2]

# initial values
def Parcel_SD_test(SD_num):
    RH_init = .95
    T_init  = 288.
    p_init  = 100000.
    ...
    for j in range(len(mu)):
        ...
        for w in w_list:
            ...
            output_folder="/home/piotr/Piotr/parcel/priv_codes/GL_II_2021/wyniki_momenty_2021"
            ...
            plt.legend(loc='best', title="updraft velocity [m/s]", frameon=1)
            ...
            ax.set_ylabel('Z [m]')
            plt.grid()
            plt.plot(mom3 * 4. / 3 * math.pi * 998.2 * 1e3 , z, label=str(w), linewidth=2, color=col[i])
            ...
            plt.grid()
            plt.ylim((0, 300))
            plt.plot(mom3*1e6, z, label=str(w), linewidth=2, color=col[i])
            plt.legend(loc='upper left', title="Cond sub-step", frameon=1)
...
Parcel_SD_test(1000)
Parcel_SD_test(10000)
Parcel_SD_test(100000)
